...
from mvckivy.uix.behaviors.adaptive_behavior import MKVAdaptiveBehavior
...
class MKVSnackbarManager(MKVAdaptiveBehavior, Widget):
    ...
    def _save_notification_data(self, notification: MKVSnackbar):
    ...
    def add_widget(self, widget, index=0, canvas=None):
        if isinstance(widget, MKVSnackbar):
            ...
            raise WrongWidgetTypeException(
                f"NotificationManager must contain {MKVSnackbar.__class__} instances only"
            )
...
class MKVSnackbar(MDSnackbar):
    def __init__(
        ...
        **kwargs,
        ...
        self._manager: MKVSnackbarManager = self.__get_notification_manager()
        super().__init__(*args, **kwargs)

    @staticmethod
    def __get_notification_manager():
        for widget in Window.children:
            if isinstance(widget, MKVSnackbarManager):
                return widget
